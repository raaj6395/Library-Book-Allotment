...
export function UserTable({ users, allocationsByUser, booksMap }) {
  // Sort users by priority for display
  const priorityOrder = [
  ...
  const getPriority = (user) => {
    const idx = priorityOrder.findIndex(
      p => p.degree === user.degree && p.year === user.year
    );
                ...
                >
                  ...
                  <TableCell className="font-medium text-foreground">
                    <div className="flex items-center gap-2">
                      {user.name}
                      {user.cpi >= 9.0 && (
                        <Award className="w-4 h-4 text-gold" title="High CPI" />
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1 max-w-xs">
                      {user.preferences.slice(0, 4).map((bookId, i) => (
                        <span
                          key={bookId}
                          className={`text-xs px-1.5 py-0.5 rounded ${
                            allocatedBookId === bookId
                    ...
                    {isAllocated ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">
                          ...
                          {allocatedBook && (
                            <span className="text-foreground"> – {allocatedBook.name}</span>
                          )}
                        </span>
                      </div>
                    ) : (
